#!/usr/bin/env node
/**
 * Generate interactive business-flow visualization data from the business-flow docs.
 *
 * The markdown documents are the single source of truth: each flow chapter embeds
 * one or more ```mermaid flowchart``` blocks. The chapters live in
 * docs/BUSINESS-FLOWS.md (index + ch. 0/18-22) and its split files under
 * docs/business-flows/ — both are scanned, index first, then split files in
 * filename order. This script extracts the diagrams, builds a structured index
 * (flows + diagrams + service participation + cross-flow edges) and emits
 * docs/flow-viz/flow-viz-data.js for the standalone interactive page.
 *
 * Run from repo root:
...
const splitDir = path.join(repoRoot, "docs/business-flows");
...
/**
 * Business-flow sources to scan, in order. The main index file goes first
 * (it owns ch. 0 and ch. 18-22); then every split file in filename order,
 * which is also chapter order. A chapter "source" label is recorded per flow
 * so the interactive page can link back to the file that holds the prose.
 */
function listSourceFiles() {
  const files = [{ rel: "docs/BUSINESS-FLOWS.md", abs: sourceMd }];
  if (fs.existsSync(splitDir)) {
    for (const name of fs.readdirSync(splitDir).filter((f) => f.endsWith(".md")).sort()) {
      files.push({ rel: `docs/business-flows/${name}`, abs: path.join(splitDir, name) });
    }
  }
  return files;
}
...
function buildData(sources, nowIso) {
  const flows = [];
  const serviceFlows = new Map(); // service -> Set<number>
  const serviceEdges = new Map(); // "from|to" -> Set<number>
  const allWarnings = [];

  const chapters = sources.flatMap((src) => parseChapters(src.md).map((c) => ({ ...c, source: src.rel })));
  ...
  for (const ch of chapters) {
    ...
    flows.push({
      ...
      anchor: ch.anchor,
      source: ch.source,
      sourceAnchor: `${ch.source}#${ch.anchor}`,
      diagrams,
    });
  }
  ...
  const problems = [];
  // Chapters 0-17 must carry at least one diagram. Chapter 0 lives in the
  // main index file; 1-17 live in docs/business-flows/. Chapters 18-22 and 23+
  // are allowed to be diagram-free (18 has the global graph, 19-22 are tables).
  for (const f of flows) {
    if (f.number <= 17 && f.diagrams.length === 0) {
      allWarnings.push(`[ch${f.number}] "${f.title}" has no mermaid diagram yet (0-17 should each have one)`);
    }
  ...
  return {
    data: {
      ...
      sourceFiles: sources.map((s) => s.rel),
      generatedAt: nowIso,
      flows,
      services,
      globalMermaid,
...
function serialize(data) {
  const header = [
    "// Generated by docs/agent/scripts/build-flow-viz.mjs from docs/BUSINESS-FLOWS.md + docs/business-flows/*.md.",
...
const sources = listSourceFiles().map((f) => ({ rel: f.rel, md: fs.readFileSync(f.abs, "utf8") }));
const { data, warnings, problems } = buildData(sources, new Date().toISOString());
...
if (args.includes("--check")) {
  ...
  if (existing !== expected) {
    console.error(
      `${path.relative(repoRoot, outFile)} is out of sync with the business-flow docs (docs/BUSINESS-FLOWS.md + docs/business-flows/*.md). Run: node docs/agent/scripts/build-flow-viz.mjs`
    );
    process.exit(1);
  }
  if (problems.length) {
