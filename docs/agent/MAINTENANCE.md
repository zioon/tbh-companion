...
pnpm run flow-viz                # regenerate docs/flow-viz/flow-viz-data.js from the business-flow docs
...
| `docs/flow-viz/flow-viz-data.js` | After adding/changing a ` ```mermaid ` diagram in `docs/BUSINESS-FLOWS.md` or any file under `docs/business-flows/`, or editing `SVC_NAMES` in `docs/agent/scripts/build-flow-viz.mjs` |
...
| Diagrams / service names in `docs/BUSINESS-FLOWS.md` or `docs/business-flows/*` | Run `pnpm run flow-viz`; commit `docs/flow-viz/flow-viz-data.js`; keep shared-service node labels matching `SVC_NAMES` (see BUSINESS-FLOWS.md ch. 21) |
| Flow prose (any `docs/business-flows/*.md` chapter) | Edit the file that owns that chapter — the routing table in `docs/BUSINESS-FLOWS.md` maps §N → file. Add a new chapter as a new file plus a new routing row; chapter numbers are stable, never renumber. |
