...
function WebChestCard({ box, heldQuantity }: { box: StageBoxCatalogItem; heldQuantity: number }) {
  ...
  return (
    <Card padding="compact" className="flex h-full flex-col gap-1.5">
      <div className="relative">
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0 flex-1">
            <p className="m-0 truncate text-[13px] font-medium text-fg">{name}</p>
            <p className="m-0 truncate text-[11px]" style={{ color: gradeColor(box.grade) }}>
              {gradeLabel(box.grade, tLookup)}
            </p>
          </div>
        </div>
      </div>
      {rangeLabel ? <p className="m-0 text-[11px] text-muted">{rangeLabel}</p> : null}
    </Card>
  );
}
      ...
      {heldRows.length > 0 ? (
        <section className="flex flex-col gap-2">
          <div className="flex flex-col gap-1">
            <h2 className="m-0 text-sm font-semibold text-fg">{t("heldHeading")}</h2>
