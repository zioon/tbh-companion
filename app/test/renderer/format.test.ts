...
describe("fmtClock", () => {
  it("zero-pads single-digit hours for column alignment", () => {
    ...
    expect(fmtClock(epoch)).toBe("01:12:15");
  });

  it("uses 24-hour time for afternoon hours", () => {
    const d = new Date(2026, 5, 16, 23, 5, 9);
    const epoch = d.getTime() / 1000;
    expect(fmtClock(epoch)).toBe("23:05:09");
  });

  it("keeps two-digit hours unchanged", () => {
    const d = new Date(2026, 5, 16, 11, 5, 9);
    const epoch = d.getTime() / 1000;
    expect(fmtClock(epoch)).toBe("11:05:09");
  });
});
