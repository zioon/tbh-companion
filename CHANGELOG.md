...
- **新增「宠物」页**：与桌面端同一套逻辑 —— 被动加成汇总、已装备/已解锁/DLC 状态徽章、击杀进度条与剩余击杀数，每只宠物还能展开看最佳刷取关卡（出现率与每次清怪期望击杀）。只需要拖入存档就能看，不必下载桌面版。
- **图鉴详情侧栏补全**：物品详情现在显示合成路径（所需材料与数量）、「被用于」的配方列表、供品掉落与掉落来源；点宝箱卡片也能看到每档掉率与刷取关卡 —— 此前这些区块在网页上是留空的。
- **宝箱页新增持有容量卡**：六类宝箱各有一张容量卡 —— 容量条（基础容量 + 符文加成）、剩余槽位与自动开门耗时，与桌面端同一套计算。
- **交易页新增筛选**：类型、品质、搜索、排序（价格/名称/品质/类型）一应俱全，另有交易专属的「仅有价」开关；价格排序下无价物品始终沉底，KPI 汇总卡保持全集口径。
- **宝箱可以按类别和等级筛选，并且能点开看掉落详情了**：目录页加了与桌面端同款的筛选（类别多选 + 等级区间）；点任意宝箱卡打开侧栏，里面是同一份掉落列表（含掉率）、农场关卡与首通关卡，点掉落物还能继续跳到物品详情。宝箱掉落数据改为按需加载（约 76 KB，一次缓存），不会拖慢其它页面。
- **交易页的物品可以点开详情了**：和图鉴页同一套交互——点物品名打开侧栏详情卡，悬停出物品小卡。浏览器拿不到的数据（掉落来源、合成路线、用途）在详情里如实省略，物品本身的图鉴信息照常显示。
- **新增 Steam 市场货币切换**：页头可以选显示货币（从价格快照带汇率的币种里选），物品栏的市值、价格列与费率，交易页的 KPI 和表格，以及图鉴的价格会一起换算并按所选货币格式化。快照没有汇率的币种不会出现在列表里，避免「选了却显示成美元」。
- **修复「持有宝箱」数量错误**：宝箱列表把每个宝箱当成一条独立的 ×1，而不是把同类宝箱合并成一个总数 —— 21 个宝箱会显示成 21 行「×1」。现在与桌面端走同一份聚合逻辑，按类别分组、数量正确。
- **物品栏现在会显示 Steam 市场价格**：此前载入存档后所有价格列都是「未加载」，即使价格快照已经就绪。现在物品栏与图鉴、交易页共用同一份同源价格快照来给每行计价，顶部的「市场价值」也会跟着算出来。浏览器拿不到的数据（收购单深度、立即卖出价）仍如实显示为未加载，而不是编一个数。
- **站点全面改版**：首页 / 物品栏 / 宝箱 / 图鉴 / 交易五页重做了视觉层次 —— 更深的底色分级与描边、页头顶部氛围光、新的页头（渐变品牌标 + 居中导航 + 语言切换）与页脚；金额、数量与存档路径统一改用等宽数字，价格与速率读起来不再跳动。内容一处没少，全部状态（无存档、读档失败、价格缺失、筛选无结果）保留。
- **首页重排**：标题改为左对齐并加了品牌眉标；三步操作改用编号卡片；免存档的三个入口加了跳转箭头；桌面版能力卡改为三列。
- **宝箱页**：目录与「持有宝箱」改为三列网格，每张卡片把品质与掉落区间并在一行。
- **交易页**：表格增加「类型」列，原先的三列不再拉得过疏。

### 桌面端

- 内置数据目录同步到游戏 v1.2.8（版本头刷新）。
- 设计变量与设计系统组件是桌面端与网页端共用的，因此本次同步更新：卡片圆角与顶部高光、按钮圆角与主要按钮辉光、提示条与统计金额卡的排版。**纯视觉调整，行为不变。**
...
- **修复「实时」功能长时间运行后内存持续膨胀**（实测从约 300 MB 涨到 1.5 GB）：读取游戏进程内存的辅助进程每读一次内存都会重新声明一次系统调用，而每次声明都会在原生内存里留下约 176 字节且永不回收 —— 按每秒数千次读取计算，约每小时泄漏 1 GB。现在系统调用只声明一次并复用，内存不再随游玩时长增长。
- 同时消除了**扫描取偏移时的内存峰值**：扫描整个游戏地址空间时改为复用同一块缓冲区（此前每个分块都新分配 4 MiB，一次扫描会留下数百 MB 未归还系统的内存）。
- **少做一次无用功**：游戏版本变化后若某些偏移在存档格式上本就无法推导，开箱事件不再每次启动都重跑一遍完整扫描（此前每次启动都要卡住约 48 秒）。

### 网页版

- **修复切换语言后已载入的存档不刷新**：在网页版里载入存档后再切换语言，「图鉴」已是新语言而「物品栏」仍是旧语言。现在语言变化会重新解析已载入的存档，两处一致。
